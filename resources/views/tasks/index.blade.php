...
<div class="min-h-screen bg-gray-50/50 pb-12">
    ...
    @if(session('success'))
        <div class="max-w-6xl mx-auto px-4 mt-6">
            <div class="bg-green-100 border border-green-400 text-green-700 px-4 py-3 rounded-xl relative shadow-sm animate-bounce">
                ...
                <table class="w-full text-left border-collapse">
                    <thead class="bg-gray-50 border-b border-gray-100">
                        <tr>
                            ...
                            <th class="px-6 py-4 text-xs font-semibold text-gray-500 uppercase text-center">Priorité</th>
                    ...
                    <tbody class="divide-y divide-gray-50">
                        @forelse ($tasks as $task)
                            <tr class="hover:bg-blue-50/30 transition-colors">
                                <td class="px-6 py-4">
                                    <a href="{{ route('tasks.show', $task) }}" class="font-semibold text-gray-800 hover:text-blue-600 transition">
                                        {{ $task->title }}
                                    </a>
                                </td>
                                <td class="px-6 py-4">
                                    <div class="text-sm font-medium text-gray-700">{{ $task->due_date->translatedFormat('d M, H:i') }}</div>
                                    <div class="text-xs text-gray-400">{{ $task->due_date->diffForHumans() }}</div>
                                </td>
                                <td class="px-6 py-4 text-center">
                                    <span class="px-3 py-1 text-xs font-bold rounded-full {{ $task->priority == 'high' ? 'bg-red-100 text-red-600' : ($task->priority == 'medium' ? 'bg-yellow-100 text-yellow-600' : 'bg-blue-100 text-blue-600') }}">
                ...
                @forelse ($tasks as $task)
                    <div class="p-5 hover:bg-gray-50 transition">
                        <div class="flex justify-between items-start mb-3">
                            <a href="{{ route('tasks.show', $task) }}" class="font-bold text-gray-900 text-lg">{{ $task->title }}</a>
                            @include('tasks.partials.status-badge', ['task' => $task])
                        </div>
                        ...
                        <div class="flex items-center justify-between">
                            <span class="text-xs font-bold uppercase {{ $task->priority == 'high' ? 'text-red-600' : 'text-gray-400' }}">
