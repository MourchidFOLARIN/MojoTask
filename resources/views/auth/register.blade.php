...
<div class="min-h-screen flex items-center justify-center bg-gradient-to-br from-green-50 to-blue-50 px-4 py-12">
    <div class="w-full max-w-md bg-white shadow-2xl rounded-[2.5rem] p-8 md:p-10 border border-white/50">
        ...
        @if(session('error'))
            <div class="mb-6 p-4 bg-red-50 text-red-600 text-sm rounded-2xl border border-red-100 flex items-center gap-2">
        ...
        <form method="POST" action="{{ route('register') }}" class="space-y-4">
            ...
            {{-- Nom --}}
            <div>
                <label for="name" class="block text-sm font-bold text-gray-700 mb-1 ml-1">Nom complet</label>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    <label for="password" class="block text-sm font-bold text-gray-700 mb-1 ml-1">Mot de passe</label>
                    <input type="password" name="password" id="password" required
                        placeholder="••••••••"
                        class="w-full px-5 py-4 bg-gray-50 border-gray-100 rounded-2xl focus:bg-white focus:ring-4 focus:ring-blue-100 focus:border-blue-500 transition-all outline-none">
                </div>
                <div>
                    <label for="password_confirmation" class="block text-sm font-bold text-gray-700 mb-1 ml-1">Confirmation</label>
